...
public open class AnySerializer : KSerializer<Any> {
    private val encodingCache = WeakHashMap<Class<out Any>, SerializationStrategy<Any>>() // TODO weak map + class = useless
    ...
    /**
     * Uses a method to provide a nullable serializer for the given [serializer]. Helps to not use the [AnySerializer.nullable], so it does not loop on itself.
     */
    ...
    /**
     * **During encoding**, called when avro4k did not find the corresponding [type]'s serializer in the [com.github.avrokotlin.avro4k.Avro.serializersModule].
     *
    ...
    protected open fun SerializersModule.resolveDeserializationStrategy(writerSchema: Schema, configuration: AvroConfiguration): DeserializationStrategy<Any> {
        ...
        return when (writerSchema.type) {
            ...
            Schema.Type.UNION -> throw UnsupportedOperationException(
                "Do not directly decode values with ${AnySerializer::class}. " +
    ...
    /**
     * **During decoding**, intercepts the process to provide a serializer for the given [writerSchema] before checking its type.
    ...
    /**
     * **During decoding**, provide the serializer for the given enum schema.
    ...
    /**
     * **During decoding**, provide the serializer for the given fixed schema.
    ...
    /**
     * **During decoding**, provide the serializer for the given record schema.
     ...
     *
     * Important notes:
     * For recursively resolve a subtype, provide a nullable [AnySerializer] to allow resolving the schema at runtime depending on the [writerSchema].
