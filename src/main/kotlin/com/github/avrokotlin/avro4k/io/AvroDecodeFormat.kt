...
sealed class AvroDecodeFormat {
    abstract fun <T> createInputStream(
        source: InputStream,
        converter: (Any) -> T,
    ): AvroInputStream<T>
    ...
    data class Data(val writerSchema: Schema?, val readerSchema: Schema?) : AvroDecodeFormat() {
        ...
        override fun <T> createInputStream(
            source: InputStream,
            converter: (Any) -> T,
        ) = when {
    ...
    data class Binary(val writerSchema: Schema, val readerSchema: Schema) : AvroDecodeFormat() {
        ...
        override fun <T> createInputStream(
            source: InputStream,
            converter: (Any) -> T,
        ) = AvroBinaryInputStream(source, converter, writerSchema, readerSchema)
    }
    ...
    data class Json(val writerSchema: Schema, val readerSchema: Schema) : AvroDecodeFormat() {
        ...
        override fun <T> createInputStream(
            source: InputStream,
            converter: (Any) -> T,
        ) = AvroJsonInputStream(source, converter, writerSchema, readerSchema)
    }
}