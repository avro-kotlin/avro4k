...
class AvroDocSchemaTest : WordSpec({
  ...
  @Serializable
  data class FieldAnnotated(@AvroDoc("hello its me") val str: String, @AvroDoc("I am a long") val long: Long, val int: Int)

  @Serializable
  data class Nested(@AvroDoc("b") val foo: String)

  @Serializable
  data class NestedAnnotated(@AvroDoc("c") val nested: Nested)
}
