...
class UUIDEncoderTest : FunSpec({
   ...
   test("encode uuids") {
      ...
      Avro.default.encodeToGenericData(UUIDTest(uuid)) shouldBeContentOf
   ...
   test("encode lists of uuids") {
      ...
      val actual = Avro.default.encodeToGenericData(UUIDList(listOf(uuid1, uuid2)))
   ...
   test("encode nullable uuids") {
      ...
      Avro.default.encodeToGenericData(NullableUUIDTest(uuid)) shouldBeContentOf ListRecord(schema, Utf8(uuid.toString()))
      Avro.default.encodeToGenericData(NullableUUIDTest(null)) shouldBeContentOf ListRecord(schema, null)
   }
}) {
   @Serializable
