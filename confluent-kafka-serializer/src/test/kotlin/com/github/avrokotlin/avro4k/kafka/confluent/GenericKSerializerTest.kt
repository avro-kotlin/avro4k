...
class GenericKSerializerTest : StringSpec() {
    init {
        ...
        "RECORD should be deserialized as GenericRecord" {
            @Serializable
            data class TheRecord(val field1: String, val field2: Int?, @AvroFixed(3) val fixed: ByteArray)

            val schema = Avro.schema<TheRecord>()
            val bytes = Avro.encodeToByteArray(schema, TheRecord("Hello", null, byteArrayOf(1, 2, 3)))

            Avro.decodeFromByteArray(schema, GenericKSerializer(), bytes) shouldBe GenericData.Record(schema).apply {
                put("field1", "Hello")
                put("field2", null)
